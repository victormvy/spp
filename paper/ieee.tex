...
%	\textcolor{red}{The backpropagation algorithm is a stochastic gradient descent algorithm that minimises a predefined loss function. It has been used in the last years in many works~\cite{leonard1990improvement,yu1995dynamic,krizhevsky2012imagenet,de2018weighted} for training shallow and deep neural networks. It updates the weights of each layer after backpropagating the loss function gradients through the network. The learning rate hyper-parameter controls the strength of the changes that are applied to the weights. Some works have checked the importance of finding the optimal value for this parameter~\cite{senior2013empirical} and have tried different approaches to improve the training process~\cite{smith2017cyclical}.}
%	
%	\textcolor{red}{Batch normalization is another technique used for this kind of networks. It reduces the internal covariate shift by normalizing layer inputs. It was presented in 2015~\cite{ioffe2015batch} and gives a critical enhancement during the training phase. Batch size is another important hyper-parameter that should be adjusted as it affects the weight updates and also the normalization~\cite{keskar2016large}\cite{radiuk2017impact}.}
%	
%	The softmax function is widely used for the output layer in neural networks for classification tasks. It is a simple and efficient function for multi-class problems, but it is not the best when working with ordinal data. In this paper, the Cumulative Link Model (CLM)~\cite{gutierrez2016ordinal,agresti2010analysis} will be used instead of the softmax function. Different link functions will be explored to compare their performance. Also, the influence of other parameters like the learning rate of the optimization algorithm and the batch size will be studied. We will contrast the results obtained with statistical analysis to provide more robust conclusions. An approximated ANOVA III \cite{miller1997beyond} test followed by a posthoc Tukey's test \cite{tukey1949comparing} will be performed because of the limitations of the computational time required to run a higher number of executions.
	...
	\begin{equation}
	\textcolor{blue}{\text{QWK} = 1 - \frac{\sum_{i,j} \omega_{i,j} O_{i,j}}{\sum_{i,j} \omega_{i,j} E_{i,j}},}
	\end{equation}
	\textcolor{blue}{where $\omega$ is the penalization matrix, $O$ is the confusion matrix and $E$ is the normalized outer product between the prediction and the true vector.}
	
	\textcolor{blue}{Then, they provided the QWK loss ($\text{QWK}_l$) definition below:}
	\begin{equation}
	\textcolor{blue}{\text{QWK}_l = 1 - \frac{\sum_{k=1}^N \sum_{c=1}^C \omega_{t_k, c} P_c(X_k)}{\sum_{i=1}^C \frac{N_i}{N} \sum_{j=1}^C ( \omega_{i,j} \sum_{k=1}^N P_j(X_k) ) },}
	\end{equation}
	\textcolor{blue}{where $X_k$ is the input data of the $k$th sample, $N$ is the number of samples, $N_i$ is the number of samples of the $i$th class and $P_c(X_k)$ is the conditional probability that the $k$th sample belongs to class $c$ given that the true class is $t_k$.}
	...
	These three types of links are explained below and represented in Figure \ref{fig:linkfunctions}. They all follow the same form $P(y \prec \mathcal{C}_q) = \Phi(b_q - \textcolor{blue}{f(\mathbf{x})})$ for a continuous cdf~$\Phi$.
	...
	\begin{itemize}
		...
		\begin{equation}
		\begin{aligned}
		\text{logit}[P(y \preceq \mathcal{C}_q)] = \log\frac{P(y \preceq \mathcal{C}_q)}{1 - P(y \preceq \mathcal{C}_q)}=& \\ = b_q - \textcolor{blue}{f(\mathbf{x})}, \quad q = 1, ..., Q-1,
		\end{aligned}
		\label{eq:logit}
		\end{equation}		
		or the equivalent expression:		
		\begin{equation}
		P(y \preceq \mathcal{C}_q) = \frac{1}{1 + e^{-(b - \textcolor{blue}{f(\mathbf{x})})}}.
		\label{eq:logit2}
		\end{equation}
		...
		\begin{equation}
		\begin{aligned}
		&\Phi^{-1}[P(y \preceq \mathcal{C}_q)] = b_q - \textcolor{blue}{f(\mathbf{x})}, &q = 1, ..., Q-1\\
		&P(y \preceq \mathcal{C}_q) = \Phi(b_q - \textcolor{blue}{f(\mathbf{x})}), &q = 1, ..., Q-1,
		\end{aligned}
		\label{eq:probit}
		\end{equation}		
		which can also be expressed as:
		\begin{equation}
		P(y \preceq \mathcal{C}_q) = \int_{-\infty}^{b_q - \textcolor{blue}{f(\mathbf{x})}} \frac{1}{\sqrt{2\pi}} e^{-\frac{1}{2}x^2} \mathrm{d}x.
		\label{eq:probit2}
		\end{equation}
		...
		that is:
		\begin{equation}
		P(y \preceq \mathcal{C}_q) = 1 - e^{-e^{b_q - \textcolor{blue}{f(\mathbf{x})}}}, \quad q = 1, ..., Q-1.
		\label{eq:cloglog2}
		\end{equation}
	\end{itemize}
	...
	\begin{equation}
	\text{link}[P(y \preceq \mathcal{C}_q)] = -\text{link}[1 - P(y \preceq \mathcal{C}_q)],
	\end{equation}
	which means that the response curve for $P(y \preceq \mathcal{C}_q)$ is symmetric around the point $P(y \preceq \mathcal{C}_q) = 0.5$, i.e. $P(y \preceq \mathcal{C}_q)$ has the same rate when approaching 0 than when approaching 1. This symmetric property can be demonstrated as follows:	
	\begin{enumerate}
		\item Let $P(y \preceq \mathcal{C}_q) \equiv p$. For the \texttt{logit} function, we have:
		\begin{equation}
		\begin{aligned}
		...
		\begin{equation}
		\begin{aligned}
		p \equiv P(y \preceq \mathcal{C}_q) &= \Phi(b_q - \textcolor{blue}{f(\mathbf{x})}) =\\
	...
	The best mean QWK value was obtained with the complementary log-log link function using a batch size of 10 and a learning rate of $10^{-3}$. However, the best CCR value was obtained with a batch size of 15, the \texttt{logit} link and a learning rate of $10^{-4}$. The optimal configuration depends on the metric we are analysing. In this case, as we are working with an ordinal problem, the most reliable metric is the QWK. \textcolor{red}{No se entonces si tiene sentido poner el resto de métricas???} \textcolor{blue}{Están puestas para comparar con artículos que no dan el QWK.}
...
%	The results over the test set show that the best link function is the complementary log-log, though it does not present significant differences with respect to the probit function ($\text{p-value} = 0.248$). There are differences between the complementary log-log and the probit function with respect to the logit function (p-values $0.000$ and $0.011$, respectively). A similar methodology has been applied to the different values of the learning rate. Significant differences can be observed in the mean values for $\eta = 10^{-3}$ with respect to the other values of the parameter (p-values $0.023$ for $10^{-4}$ and $0.000$ for $10^{-2}$). Also, there are differences for $\eta = 10^{-4}$ concerning $\eta = 10^{-2}$ ($\text{p-value} = 0.000$).
...
%	The results showed that the best link function is the logit one, in this case, with $0.827$ as mean QWK value. Also, there are significant differences between this function and the probit link ($\text{p-value} = 0.000$), that has a mean QWK value of $0.781$. Both links functions present significant differences concerning the complementary log-log function, which has a mean value of $0.743$.
...
%		\begin{tabular}{cccc}
%			\hline\hline
%			       LF         & \texttt{logit} & \texttt{probit}  & \texttt{clog-log} \\ \hline
%			 \texttt{logit}   &    $0.827$     & $\mathbf{0.000}$ & $\mathbf{0.000}$  \\
%			 \texttt{probit}  &       -        &     $0.781$      & $\mathbf{0.000}$  \\
%			\texttt{clog-log} &       -        &        -         &      $0.743$      \\ \hline\hline
%			       LR         &   $10^{-2}$    &    $10^{-3}$     &     $10^{-4}$     \\ \hline
%			    $10^{-2}$     &    $0.818$     & $\mathbf{0.000}$ & $\mathbf{0.000}$  \\
%			    $10^{-3}$     &       -        &     $0.864$      & $\mathbf{0.000}$  \\
%			    $10^{-4}$     &       -        &        -         &      $0.670$      \\ \hline\hline
%			       BS         &       64       &       128        &        256        \\ \hline
%			       64         &    $0.761$     & $\mathbf{0.026}$ & $\mathbf{0.000}$  \\
%			       128        &       -        &     $0.802$      & $\mathbf{0.000}$  \\
%			       256        &       -        &        -         &      $0.788$      \\ \hline\hline
%		\end{tabular}
		\begin{tabular}{cccc}
			\hline\hline
			       LF         &        LF         & Mean diff. &  Sig.   \\ \hline
...
%	The same methodology was followed for the learning rate and the batch size factors. The test reported significant differences between $\eta=10^{-3}$ (mean value $0.864$) and the rest of values of this parameter ($\text{p-value} = 0.000$ for both values), being this one the best value for this factor. The second best value is $10^{-2}$, which has a mean QWK value of $0.818$. Both values present significant differences with $\eta=10^{-4}$, which has a mean QWK value of $0.670$ ($\text{p-value = 0.000}$).
%	
%	Finally, the best value for the batch size parameter is $128$ (mean value $0.802$) as it shows differences with the rest of values (p-values $0.026$ and $0.000$). The second best value is $256$ (mean value of $0.788$), and it has significant differences with respect to $64$ ($\text{p-value} = 0.000$), with a mean value of $0.761$.
	...
	Finally, a comparison of the best \textcolor{blue}{test results} for each dataset for ordinal and nominal cases and previous works is shown in tables \ref{table:ComparisonDR} and \ref{table:ComparisonAdience}. The proposed ordinal model outperforms all the other alternatives in terms of QWK. The performance gain of CLM over the softmax reaches $16.8\%$ for Diabetic Retinopathy and $11.9\%$ for Adience dataset. The improvement of the ordinal method for Retinopathy dataset is higher than for Adience dataset. It seems that the method proposed in this work offers a more significant improvement as the given problem complexity increases.
